...
- **Routes**: `/` (Money — balances + spend permissions, default landing after wallet connect), `/activity` (Activity Feed with trusted destinations rail), `/defi` (DeFi — Swap to Pay via Uniswap), `/agent` (Agent — BYOA identity + presence), `/rules` (Rules — spending constraints + Venice privacy controls), `/receipt/:id` (Receipt Detail)
